...
impl ConfigData {
    pub fn new(regions: Vec<Region>, graph: PortGraph) -> Self{
        Self { regions, graph}
    }
}


pub fn load_config_data<P>(regions_data_file_path: P, connections_data_file_path: P) -> Result<ConfigData, Box<dyn Error>> where P: AsRef<Path> {
...
#[cfg(test)]
mod tests {
    use crate::{config::load_config_data, region::PortID};
